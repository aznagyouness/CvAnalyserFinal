...
class DeepSeekModel(LLMInterface):
    ...
    async def generate_text_stream(
        ...
        **kwargs
        ...
        """
        Asynchronously generates a stream of responses from DeepSeek.
        If documents are provided, it builds a RAG-optimized prompt.
        """
        if not self.generation_model_id:
            self.logger.error("Generation model ID was not set.")
        ...
        try:
            ...
            self.logger.error(f"Error during DeepSeek streaming generation: {str(e)}")
            yield f"\n[STREAM_ERROR: {str(e)}]"
    ...
    async def embed_text(self, text: Union[str, List[str]], **kwargs) -> Union[List[float], List[List[float]]]:
        """
        Asynchronously generates embeddings using DeepSeek.
        ...
        # Using a safe batch size of 50 for DeepSeek/compatible endpoints
        batch_size = 50
        all_embeddings = []
        ...
        try:
            ...
            for i in range(0, len(input_texts), batch_size):
                ...
                if not response or not response.data:
                    self.logger.error(f"DeepSeek embedding API returned empty data for batch {i//batch_size}.")
                    continue
